...
///
/// Phenogram compatibility patch: a pinned TDLib `HttpReader` accepts a
/// complete request head up to 256 KiB without imposing a separate header-count
/// limit. More than 32,768 short, distinct fields can fit in that byte budget,
/// so the map indices must not truncate them.
type Size = u32;

/// A raw capacity of 2^17 leaves 98,304 usable slots, which is above the
/// greatest syntactically possible field count inside the official 256 KiB
/// request-head limit.
const MAX_SIZE: usize = 1 << 17;
...
/// Hash values use the same width as map positions. While `fast_hash` and
/// `Hasher` return `usize` hash codes, limiting the effective hash code to the
/// map's addressable bits is sufficient because `indices` never grows beyond
/// `MAX_SIZE`.
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
...
impl<T> HeaderMap<T> {
    ...
    pub fn try_reserve(&mut self, additional: usize) -> Result<(), MaxSizeReached> {
        ...
        // elements is bounded by MAX_SIZE.
        let cap = self
            .entries
            .len()
