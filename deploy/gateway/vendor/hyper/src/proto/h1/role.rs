...
// Phenogram compatibility patch: match the pinned TDLib `HttpReader` total
// request-head bound instead of rejecting otherwise valid names at 64 KiB.
const MAX_HEADER_NAME_LEN: usize = 1 << 18;
#[cfg(feature = "server")]
// Phenogram compatibility patch: the pinned official Telegram Bot API HTTP
// reader accepts a request head up to 256 KiB, including a GET request target.
const MAX_URI_LEN: usize = 1 << 18;
#[cfg(feature = "server")]
// `a:\n` is the shortest syntactically useful header line. This is therefore
// the largest header count that could fit inside the official 256 KiB head
// limit, and bounds the dynamic allocation below.
const MAX_DYNAMIC_HEADERS: usize = MAX_URI_LEN / 3;
...
impl Http1Transaction for Server {
    ...
    fn parse(buf: &mut BytesMut, ctx: ParseContext<'_>) -> ParseResult<RequestLine> {
        ...
        let header_capacity = ctx
            .h1_max_headers
            .unwrap_or_else(|| dynamic_request_header_capacity(buf));
        ...
        let mut headers_indices: SmallVec<[MaybeUninit<HeaderIndices>; DEFAULT_MAX_HEADERS]> =
            if header_capacity == DEFAULT_MAX_HEADERS {
                smallvec_inline![MaybeUninit::uninit(); DEFAULT_MAX_HEADERS]
            } else {
                smallvec![MaybeUninit::uninit(); header_capacity]
            };
        {
...
#[cfg(feature = "server")]
fn dynamic_request_header_capacity(buf: &[u8]) -> usize {
    // Count only the bytes already received. On a partial request this grows
    // with each header line and prevents an early 431 at the old fixed limit;
    // on a complete request it allocates only what that request needs.
    buf.iter()
        .filter(|byte| **byte == b'\n')
        .count()
        .saturating_add(1)
        .clamp(DEFAULT_MAX_HEADERS, MAX_DYNAMIC_HEADERS)
}
...
fn record_header_indices(
    ...
    for (header, indices) in headers.iter().zip(indices.iter_mut()) {
        if header.name.len() > MAX_HEADER_NAME_LEN {
            debug!("header name larger than 256kb: {:?}", header.name);
...
mod tests {
    ...
    fn parse_too_large_headers() {
        ...
        fn parse(
            ...
            gen_size: usize,
            server_should_succeed: bool,
            client_should_succeed: bool,
        ) {
            {
                // server side
        ...
        // Phenogram dynamically sizes the default server request-header slots;
        // response parsing retains Hyper's default limit of 100.
        //
        // - less than or equal to 100, accepted
        //
        parse(None, 0, true, true);
        parse(None, 1, true, true);
        parse(None, 50, true, true);
        parse(None, 99, true, true);
        parse(None, 100, true, true);
        //
        // - more than 100, accepted by the server and rejected by the client
        //
        parse(None, 101, true, false);
        parse(None, 102, true, false);
        parse(None, 200, true, false);

        // max_headers is 0, parser will reject any headers
        //
        // - without header, accepted
        //
        parse(Some(0), 0, true, true);
        //
        // - with header(s), rejected
        //
        parse(Some(0), 1, false, false);
        parse(Some(0), 100, false, false);

        // max_headers is 200
        //
        // - less than or equal to 200, accepted
        //
        parse(Some(200), 0, true, true);
        parse(Some(200), 1, true, true);
        parse(Some(200), 100, true, true);
        parse(Some(200), 200, true, true);
        //
        // - more than 200, rejected
        //
        parse(Some(200), 201, false, false);
        parse(Some(200), 210, false, false);
    }
